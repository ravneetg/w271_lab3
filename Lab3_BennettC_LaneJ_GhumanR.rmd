...
This section helps create functions to run diagnostics
...
lmod2 = lm(PINCP_log ~ AGEP_bin + factor(SCHL_bin) + WKHP_bin + I(WKHP**2), data=acs.trainingdata)
summary(lmod2)
checkModelSig(lmod2)

# comparing second model with model 1 - checking fit
AIC(lmod1,lmod2)

# adding JWMNP - travel time to work to the model
lmod3 = lm(PINCP_log ~ AGEP_bin + factor(SCHL_bin) + WKHP_bin +  I(WKHP**2) + JWMNP_bin, data=acs.trainingdata)
summary(lmod2)
checkModelSig(lmod2)

# comparing current and previous model - checking fit
AIC(lmod2,lmod3)


# adding MAR - marital status
lmod4 = lm(PINCP_log ~ AGEP_bin + factor(SCHL_bin) + WKHP_bin +  I(WKHP**2) + JWMNP_bin + MAR_bin, data=acs.trainingdata)
summary(lmod4)
checkModelSig(lmod4)
...
# adding JWMNP - travel time to work to the model
lmod5 = lm(PINCP_log ~ AGEP_bin + factor(SCHL_bin) + WKHP_bin +  I(WKHP**2) + JWMNP_bin + MAR_bin + ANC1P + LANP_bin, data=acs.trainingdata)
summary(lmod5)
checkModelSig(lmod5)

# comparing second model with model new model - checking fit
AIC(lmod4,lmod5)
# model 5 show a little improvement based on AIC

################################################
############ code for predictions ######## 
################################################
predictions = predict.lm(lmod5,acs.small.testdata,se.fit = FALSE, interval = "confidence" )

acs.small.testdata$fit=as.vector(predictions[,1])
acs.small.testdata$lwr=as.vector(predictions[,2])
acs.small.testdata$upr=as.vector(predictions[,3])
acs.small.testdata$lPINCP=log(acs.small.testdata$PINCP)

#plotting results - actual vs fitted
plot.data=(acs.small.testdata[c("lPINCP","fit","lwr","upr")])
plot.data=plot.data[complete.cases(plot.data),]
plot(plot.data$lPINCP, type = "l",col="red")
lines(plot.data$fit,col="green")
