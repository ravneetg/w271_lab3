...
Pros of VAR models: 
VAR models are easy to estimate. 
No need to specify endogenous and exogenous variables. 
Cons: 
Multicollinearity can be a problem as more series are added to the model
As number of variables increases, the number of parameters increases by a lot.

Pros of SARIMA models: 
SARIMA considers the order in the time-series and captures historical info by using AR to take into account the past values, MA for present and future values, and differening to attain stationarity from seasonal effects.
Cons: 
SARIMA can capture only linear relationship
