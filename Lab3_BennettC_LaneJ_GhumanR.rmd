...
acsTransformed$AGEP_bin <- cut(acs$AGEP, c(15, 20, 30, 40, 50, 60, 70, 80, 90, 100))
...
acsTransformed$POBP_US = acs$POBP < 72 #changed from 73 to 72 #rg

######################################################
#Data Transformation for POWSP (Place of Work)       #
######################################################

acsTransformed$POWSP_US = acs$POWSP < 72 #changed from 73 to 72 #rg
...
acs.testdata=acs[sample(nrow(acs), 100,replace=FALSE), ]
...
modtest = lm(log(PINCP) ~ factor(AGEP_bin) + factor(SCHL_bin) + JWMNP + WKHP,data=acs.trainingdata)#rg
summary(modtest)
...
predictions = predict.lm(modtest,acs.testdata,se.fit = FALSE, interval = "confidence" )#rg
