...
#rg hist(acs$AGEP_bin)
#rg summary(acs$AGEP_bin)
#rg describe(acs$AGEP_bin)
#rg length(na.omit(acs$AGEP_bin != 0))
...
acs$AGEP_bin <- cut(acs$AGEP, c(15,18, 22, 30, 40, 50, 60, 70, 80, 90, 100))#rg
...
#rg par(mfrow=c(1,1))
acs$SCHL_bin <- as.numeric(cut(acs$SCHL, c(-1, 12, 16, 18, 20, 22, 24))) #rg
#rg table(acs$SCHL_bin)
runEDA("SCHL_bin")
makeFreqTable("SCHL_bin")
makeScatterPlot("SCHL_bin")

#rg ggplot(acs, aes(x=SCHL_bin)) + xlim(0,7) + geom_bar(stat="count", fill="#0072B2", colour="black") + ggtitle("Years of School")
...
#rg summary(acs$JWMNP_bin)
#rg length(na.omit(acs$JWMNP_bin != 0))
#rg length(acs$JWMNP_bin[acs$JWMNP_bin==0])
#rg hist(acs$JWMNP_bin)
runEDA("JWMNP_bin")
...
#rg summary(acs$WKHP_bin)
#rg length(na.omit(acs$WKHP_bin != 0))
#rg hist(acs$WKHP_bin)
runEDA("WKHP_bin")
...
#rg summary(acsTransformed$INDP)
#rg plot(acsTransformed$INDP)
runEDA("INDP_bin")
...
#rg length(is.null(acs$ANC1P_bin))
#rg summary(acs$ANC1P_bin)
...
# adding JWMNP - travel time to work to the model
lmod5 = lm(PINCP_log ~ AGEP_bin + factor(SCHL_bin) + WKHP_bin +  I(WKHP**2) + JWMNP_bin + MAR_bin + ANC1P, data=acs.trainingdata)
summary(lmod5)
checkModelSig(lmod5)
