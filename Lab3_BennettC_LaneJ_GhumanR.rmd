...
acs <- acs[acs$AGEP>=15,]
acs <- acs[!is.na(acs$WKHP),]
print("Records Remaining")
nrow(acs)
...
acs$ANC1P_bin[is.na(acs$ANC1P)] = 0
...
acs.train2 <- acs.trainingdata[sample(nrow(acs.trainingdata), 100000, replace=FALSE), ]
nrow(acs.train2)
summary(acs.train2)
...
runCV =function(model){
  ...
  predictions = predict.lm(model,acs.small.testdata, interval = "confidence" )
  predictions = exp(as.vector(predictions[,1]))
par(mfrow=c(2,1))

    scatterplot(acs.small.testdata$PINCP,predictions, 
...
MSE = mean((predictions - acs.small.testdata$PINCP)^2,na.rm = T)
...
### Attempt 5: PINCP ~ AGEP + SCHL + WKHP + JWMNP + MAR + ANC1P + LANP

Model 5 (lmod5): At this point we added in both the ancestry and the language spoken at home variables. The model now looks like this:

PINCP_log ~ AGEP_bin + factor(SCHL_bin) + WKHP_bin +  I(WKHP**2) + JWMNP_bin + MAR_bin + ANC1P_bin + LANP_bin

Results: The addition of the Ancestry and the Language variables improved the R-squared to 0.545 and the AIC showed that Model 5 provided improvement over Model 4.  However, the Ancestry variable introduced quite a few coefficients that were not significant, so this model needs further refinement. Due to the large number of insignificant Ancestry coefficients, given more time, we would collapse many of the Ancestry categories into a smaller number of bins. The Wald Test also provides evidence that the models are not the same, and that the coefficients are different than zero.  The AIC indicates that Model 5 shows an improvement over Model 4.  Model 5 is our chosen OLS Regression Model and we therefore explore the assumptions and diagnostics further.

```{r}

lmod5 = lm(PINCP_log ~ AGEP_bin + factor(SCHL_bin) + WKHP +  I(WKHP**2) + JWMNP_bin + MAR_bin + ANC1P_bin + LANP_bin, data=acs.train2)
summary(lmod5)
...
[The model violates the assumption of linearity in parameters as can be seen in the plot of the residuals against the fitted values. The remediation for this is to look for further transformations, but we feel we may have exhausted this as an option.
...
The following explanatory variables appear to have high correlation: Age and Marital Status, and Ancestry and Language Spoken at Home.  Constant variables were ruled out, as they are collinear with the intercept term. To remediate this, the following variables  will be eliminated: ADJINC. 
...
The Residuals Vs. Fitted plot as a loess smooth superimposed, is difficult to discern whether the mean is zero, even with a randomly selected training set of 100,000. The plot shows a slight curve, and it appears there might be more residual data points below the mean line of zero.  Ther eis an unusual cluster below the around -10 on the residuals line. However, if we can't meet the Assumption of Zero Conditional Mean, we might be able to meet the weaker assumption of exogeneity.
...
Because of the large size of our dataset, exogeneity is a critical assumption. We tested that the explanatory variables are uncorrelated with the error term (called exogenity). The plots of residuals vs. independent variables all appear to indicate a mean of zero, which supports the assumption of exogeneity.
...
The null hypothesis of homoskedasticity specified in the *Breusch-Pagan* test is rejected. Remediation for Heteroskedasticity is to utilize heteroskedasticity-robust tools such as the White standard errors.

###Normality of error term:

Due to the large sample size, small deviations from the assumption of a normal distribution of error terms do not necessarily negate the statistical significance of tests that require this assumption. Our model does show significant deviations in the normality of the error term, which we attempted to reduce the effects by using robust tools (i.e F-test), as well as by transforming the PINCP variable.
...
The predicted vs. actual scatterplot appears to be mostly linear.  There is a tendency for the model to underestimate the predicted variable (log of income). There are quite a few observations outside of the 95% confidence interval, which calls into question the overall efficacy of the model. Based on the predicted vs. actual plot, it appears that we might need different models for different income levels.
...
This model does not satisfy all of the CLM assumptions, however, with respect to answering the main question, we would not base predictions on chosen features alone.  There are other sources of income like inheritance, rental income etc that are not described by features like Work hours per week, Education etc alone. AGE has a high value of the coefficients followed by SCHL(years of school education) indicating higher income in the groups with more education and years of experience. Work hours per week has a very high value and is inline with the intuition that income has a direct dependency on number of hours worked per week. While the model does a good job at predicting income for the middle income group (20k-80k), the usage of model would define the practical significance of the model. For example, if the model were to be used to predict an income range for marketing, it could work well but for higher stakes decisions like approving line of credit, the risk associated with using this model goes up.

##Conclusion:

While our model is statistically significant and explains at least a lot of the variance, we found that overall a single linear regression is probably not the best fit for personal income.  We either will need more and different explanatory variables or we should employ mixture or other, possibly non-parametric, modeling techniques, such as k-nearest neighbors or neural networks.  
